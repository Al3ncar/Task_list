...
export default class ToDo extends React.Component{
  ...
  add = () =>{
    ...
    if(task !== ""){
      this.setState({
      list: list.concat({list:task, id: Date.now()}),
      task:""
    })}
  }
  clear = (id) =>{
    ...
    return(
     <Main>
        ...
        <Boxtitle>
          ...
          <Data><input type="date"/></Data>
        </Boxtitle>
        <Listbox>
          <Newtesk>
            <Del onClick={this.del}>⌦</Del>
            <Task value={task} onChange={this.handleChange} placeholder="Escreva sua Tarefa Aqui!!!"/>
            ...
            <ul>
              <List>
                ...
                <Trash onClick={() => this.clear(item.id)}
                src={trash}/>
              </List>
            </ul>))}
